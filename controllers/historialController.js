...
class HistorialController{
    consultar (req, res){
        try {
            db.query('select * from tbl_historial',
                (err, data) =>{
                    if (err){
    ...
    ingresar (req, res){
        try{
            const { idPaciente, idCita, diagnostico} = req.body;
            //query: un proceso de ejecucion de operacion
            db.query('insert into tbl_historial (idPaciente, idCita, diagnostico) values(?, ?, ?);',
                [idPaciente, idCita, diagnostico], (err, rows) =>{
                    if (err){
                        res.status(400).send(err);
                    }
    ...
    actualizar(req, res){
        ...
        try{
            const { idPaciente, idCita, diagnostico } = req.body;
            db.query('update tbl_historial set idPaciente = ?, idCita = ?, diagnostico = ? where idHistorial = ?',
                [idPaciente, idCita, diagnostico], (err, rows) =>{
                    if (err){
                        res.status(400).send(err);
                    }
